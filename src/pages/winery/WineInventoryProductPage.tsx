...
const WineInventoryProductPage = () => {
  ...
  const isCreateMode = inventoryId === undefined
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateWineInventory(inventoryId, {
        ...
        sku: form.sku.trim(),
        description: form.description.trim() || null,
        allocated_bottles: form.allocated_bottles,
        allocated_case: form.allocated_case,
        price: parsedPrice,
        available: form.available,
        image_url: nextImageUrl,
      })
        ...
        <section className="rounded-3xl bg-surface p-6 shadow-sm ring-1 ring-border">
          <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              />
              <Input
                label={t('form.allocated_bottles')}
                id="allocate_bottles"
                type="number"
                min={0}
                step={1}
                value={form.allocated_bottles}
                onChange={e => setForm(prev => ({ ...prev, allocated_bottles: Number(e.target.value) }))}
                placeholder={t('form.allocated_bottlesPlaceholder')}
                inputSize='md'
              />
              <Input
